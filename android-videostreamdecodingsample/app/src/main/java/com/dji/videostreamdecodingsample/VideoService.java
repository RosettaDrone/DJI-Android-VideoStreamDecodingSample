...
public class VideoService extends Service {
    ...
    */
    ...
    java.io.OutputStream dumpOutputStream;
    void dumpData(byte[] data, int size, int frameNum, boolean isKeyFrame, int width, int height) {
        Log.d(TAG,"frame: " + frameNum + ", w: " + width + ", h: " + height +  ", key: " + isKeyFrame + ", size: " + size + ", rSize: " + data.length);
    }

    public void onDataRecv(byte[] data, int size, int frameNum, boolean isKeyFrame, int width, int height) {
        dumpData(data, size, frameNum, isKeyFrame, width, height);
        if (size > 0 && isRunning) {
